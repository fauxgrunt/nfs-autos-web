...
export default function Navbar() {
  ...
  return (
    <>
      <nav className="sticky top-0 w-full bg-white/95 backdrop-blur-sm z-50 border-b border-slate-100 h-24 transition-all duration-300">
        <div className="max-w-7xl mx-auto flex justify-between items-center h-full px-6 lg:px-12">
            ...
            >
              ...
              <div className={`absolute top-full left-0 mt-0 w-64 bg-white border border-slate-100 shadow-xl transition-all duration-300 ${
                ...
                <div className="py-2 flex flex-col">
                  ...
                  <Link href="/sold" className="px-6 py-4 text-xs font-light tracking-widest text-slate-600 hover:text-white hover:bg-[#0f172a] transition-all uppercase border-t border-slate-50" style={{ fontFamily: 'Raleway, sans-serif' }}>Archive / Sold</Link>
                </div>
              </div>
            </div>
          ...
          <div className="lg:hidden flex items-center gap-4 relative z-50">
            <button onClick={toggleMenu} className="p-2 hover:bg-slate-50 rounded-full transition-colors group">
      ...
      {/* MOBILE MENU OVERLAY */}
      <div className={`fixed inset-0 bg-white z-40 lg:hidden transition-transform duration-500 ease-[cubic-bezier(0.76,0,0.24,1)] ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex flex-col h-full">
          <div className="flex-1 pt-32 px-8 pb-8 flex flex-col gap-6">
            ...
            <div className="mt-auto space-y-4">
              <Link href="/book" onClick={toggleMenu} className="flex items-center justify-center w-full py-5 bg-[#0f172a] text-white font-medium text-sm uppercase tracking-widest" style={{ fontFamily: 'Raleway, sans-serif' }}>Book Appointment</Link>
            </div>
          </div>
        </div>
      </div>
