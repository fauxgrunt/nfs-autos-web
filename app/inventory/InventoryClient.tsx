...
export default function InventoryClient({ initialCars }: { initialCars: any[] }) {
  ...
  // Generate dynamic filter options from initialCars (filter out falsy values)
  const availableMakes = Array.from(new Set(initialCars.map(car => car.make).filter(Boolean))).sort();
  const availableModels = Array.from(new Set(initialCars.map(car => car.model).filter(Boolean))).sort();
  const availableYears = Array.from(new Set(initialCars.map(car => (car.year ? car.year.toString() : null)).filter(Boolean))).sort((a, b) => parseInt(b) - parseInt(a));
  const availableTransmissions = Array.from(new Set(initialCars.map(car => car.transmission).filter(Boolean))).sort();
  const availableBodies = Array.from(new Set(initialCars.map(car => car.bodyType).filter(Boolean))).sort();

  // Group models by make for better organization
  const modelsByMake = initialCars.reduce((acc, car) => {
  ...
  const filteredCars = [...initialCars]
    ...
    .sort((a, b) => {
      const mileageA = a.odometer || a.mileage || 0;
      const mileageB = b.odometer || b.mileage || 0;
  ...
  return (
    <>
      ...
      <main className="flex min-h-screen w-full overflow-x-hidden pt-24">
                      ...
                      {availableMakes.map(make => {
                        ...
                        return (
                          <div key={make}>
                            ...
                            {models.map(model => (
                              <label key={model as string} className="flex items-center gap-3 cursor-pointer mb-3 group">
                                <input
                                  type="checkbox"
                                  checked={selectedModels.includes(model as string)}
                                  onChange={() => toggleModel(model as string)}
                                  className="appearance-none h-5 w-5 rounded-sm border border-gray-600 bg-transparent checked:bg-neutral-800 checked:border-neutral-800 checked:bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20fill%3D%22%23fff%22%20viewBox%3D%220%200%2016%2016%22%3E%3Cpath%20d%3D%22M12.207%204.793a1%201%200%20010%201.414l-5%205a1%201%200%2001-1.414%200l-2-2a1%201%200%20011.414-1.414L6.5%209.086l4.293-4.293a1%201%200%20011.414%200z%22%2F%3E%3C%2Fsvg%3E')] bg-no-repeat bg-center cursor-pointer transition-all duration-200 hover:border-gray-400 accent-neutral-800"
                                />
                                <span className="font-ui text-sm font-medium text-white/70 group-hover:text-white transition-colors tracking-wide">{model as string}</span>
                              </label>
                            ))}
                          </div>
                        );
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {filteredCars.map((car) => (
                <CarCard
                  key={car.id}
                  id={car.id}
                  slug={car.slug} // Pass slug to CarCard
                  title={car.title}
                  estLandedCost={car.price}
                  image={car.image}
                  mileage={car.mileage}
        ...
        {/* DESKTOP SIDEBAR */}
        <aside className="hidden lg:block w-64 bg-[#0a0a0a] text-white flex-shrink-0">
          <div className="p-6 pb-6">
            <div className="mb-8 pb-6 border-b border-white/20">
              <p className="font-ui text-sm font-semibold text-white/90 mb-3 tracking-wide">
                {filteredCars.length} <span className="text-white/60">SOURCING OPPORTUNITIES</span>
